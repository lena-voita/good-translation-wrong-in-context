...
# item[0]: src _eos ctx_n _eos ctx_{n-1} _eos ... _eos ctx_1 _eos_eos dst_ctx_n _eos ... _eos dst_ctx_1
# item[1]: dst

sent_splitter = ' _eos '
srcdst_splitter = ' _eos_eos '

def max_sent_len(sents):
    return max([len(el.split()) for el in sents.split(sent_splitter)])


def num_sents(sents):
    return len(sents.split(sent_splitter))


def multi_ctx_with_dst_weight_func(x):
    return (num_sents(x[0].split(srcdst_splitter)[0]),     # src len
            max_sent_len(x[0].split(srcdst_splitter)[0]),  # max src ctx len
            max_sent_len(x[0].split(srcdst_splitter)[1]),  # max dst ctx len
            max_sent_len(x[1]))                            # dst len


def form_adaptive_batches_mt_multi_ctx_with_dst(data, batch_len, batch_size_max=0):
    seq = iter(data)
    prev = []
    max_len = 0
    max_len_dst_ctx = 0
    ...
    while not done:
        ...
        try:
            while True:
                item = next(seq)
                max_len = max(max_len, max_sent_len(item[0].split(srcdst_splitter)[0]))
                max_len_dst_ctx = max(max_len_dst_ctx,
                                      max_sent_len(item[0].split(srcdst_splitter)[1]))
                max_dst_len = max(max_dst_len, max_sent_len(item[1]))
                max_ctx_sents = max(max_ctx_sents, num_sents(item[0].split(srcdst_splitter)[0]))
                if (max_len * (max_ctx_sents - 1) + \
                    max(max_dst_len, max_len) + \
                    max_len_dst_ctx * (max_ctx_sents - 1)) * (len(batch) + 1) > batch_len or \
                        (batch_size_max and len(batch) >= batch_size_max):
                    prev, max_len, max_len_dst_ctx, max_dst_len, max_ctx_sents = [item], \
                                                                                 max_sent_len(
                                                                                     item[0].split(srcdst_splitter)[0]), \
                                                                                 max_sent_len(
                                                                                     item[0].split(srcdst_splitter)[1]), \
                                                                                 max_sent_len(item[1]), \
                                                                                 num_sents(
                                                                                     item[0].split(srcdst_splitter)[0])
                    break
...
def form_adaptive_batches_windowed_mt_multi_ctx_with_dst(data,
    ...
    for p in data:
        if len(buf) >= split_len:
            ...
            buf = sorted(buf, key=weight_func, reverse=reverse)
            chunks = list(form_adaptive_batches_mt_multi_ctx_with_dst(buf, batch_len=max_size, batch_size_max=batch_size_max))
    ...
    buf += last_chunk
    buf = sorted(buf, key=weight_func, reverse=reverse)
    chunks = list(form_adaptive_batches_mt_multi_ctx_with_dst(buf, max_size, batch_size_max=batch_size_max))
    rng.shuffle(chunks)
    for chunk in chunks:
        yield chunk
