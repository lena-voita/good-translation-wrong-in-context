...
class Model(TranslateModelBase):
    ...
    def __init__(self, name, inp_voc, out_voc, **hp):
        ...
        # ============ LENA ===================
        assert len(
            hp['train_mode']) > 0, "you must specify train_mode like in {'sample': 1, 'noise': 0, 'beam': 0}"
        assert set(hp['train_mode'].keys()) == set(["sample", 'noise', 'beam'])
        self.train_mode = hp['train_mode']  # {"sample": 1, 'noise': 0, 'beam': 0}
        self.dropout = hp['dropout']  # {"sample": {'dropout': 0.1, 'method': 'random_word'},
        # 'noise': {'dropout': 0.2, 'method': 'random_word'},
        # 'beam': {'dropout': 0.2, 'method': 'random_word'}}
    ...
    def make_feed_dict(self, batch, is_train=False, **kwargs):
        """Data in the format:
        batch - pairs (inp, out)
        inp = beam_1 _eos ... beam_n _eos_eos ' _eos '.join(samps_1) _eos_eos ... _eos_eos ' _eos '.join(samps_n)
        out = dst_1 _eos ... dst_n"""

        no_dropout = {"sample": {'dropout': 0, 'method': 'random_word'},
                      'noise': {'dropout': 0, 'method': 'random_word'},
                      'beam': {'dropout': 0, 'method': 'random_word'}}

        mode_probs = self.train_mode if is_train else {"sample": 0, 'noise': 0, 'beam': 1}

        dropout = self.dropout if is_train else no_dropout
        ...
        def word_dropout(sent, dropout, method, voc):
            """sent is list of ids, with _EOS_"""
            drop_mask = (np.random.random(len(sent)) < dropout).astype(int)
            drop_mask[-1] = 0

            if method == 'unk':
                replacement = np.array([voc._unk] * len(sent))
            elif method == 'random_word':
                replacement = np.random.randint(low=max(voc.bos, voc.eos, voc._unk) + 1, high=voc.size(),
                                                size=len(sent))
            else:
                raise ValueError("Unknown word dropout method: %r" % method)

            result = drop_mask * replacement + (1 - drop_mask) * sent
            return list(result)

        def read_sample(sample):
            probs = []
            sents = []
            for el in sample.strip().split(splitter):
        ...
        for inp, out in batch:
            beam_sents = [el.strip() for el in inp.split(splitter_sample)[0].split(splitter)]
            out_sents = [el.strip() for el in out.split(splitter)]
            samples = inp.split(splitter_sample)[1:]
            if is_train:
                assert len(samples) == len(beam_sents) == len(out_sents)

            def get_base_translation(ind):
                keys, probs = map(list, zip(*mode_probs.items()))
                mode = np.random.choice(keys, p=probs)
                if mode == 'sample':
                    sents_, probs_ = read_sample(samples[ind])
                    base_translation = np.random.choice(sents_, p=probs_)
                elif mode == 'noise':
                    base_translation = out_sents[ind]
                elif mode == 'beam':
                    base_translation = beam_sents[ind]

                dropout_params = dropout[mode]
                base_translation = word_dropout(ids_from_sent(base_translation, use_inp_voc=False),
                                                dropout=dropout_params['dropout'],
            ...
            for i in range(len(beam_sents)):
                base_translation = get_base_translation(i)
                current_inp += base_translation[:-1]

            outs.append(ids_from_sent(out, use_inp_voc=False))
            inps.append(current_inp[:-1] + [ieos])

            reverse = 0
            if self.hp.get('dst_reverse', 0) != 0:
    ...
    def _get_batch_sample(self):
        src_line = 'я видел кота . _eos он был серый _eos еще он был голодный _eos_eos я видела кота .:2 _eos я видел кошку:1 _eos_eos она была сер `ая:2 _eos она сер `ая:1 _eos_eos и голод `ная:4 _eos он был голоден:3 _eos и еще голод `ная:1'
        dst_line = 'я видел кошку . _eos она сер `ая _eos и голод `ная'
