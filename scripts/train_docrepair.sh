...
#      main model hp,
#      minor model hp (for the DocRepair model, we set "share_emb": True since source and target vocabularies are the same)
#      regularization and label smoothing
#      inference params (beam search with a beam of 4)
#      DocRepair-specific parameters (below are the ones you may want to change)
